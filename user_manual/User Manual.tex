...
Univariate multiclass supervised time series classification is implemented in {\TSAT} with the \textbf{Representative Pattern Mining} or \textbf{RPM} for short algorithm~\cite{wang2016rpm}.  But before describing RPM here are some definitions to some common machine learning terms that will be used:
...
\begin{description}
	...
	\item[Multivariate] Multivariate, multi-attribute, or multi-feature means that each example is represented by multiple attributes.  This means multiple time series in time series classification is defined to be an example.
\end{description}



RPM works is by identifying appropriate Sliding window size, PAA size, and alphabet length and it identifies the motifs that match the class it belongs to more so than the other classes.
